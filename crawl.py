	...
	for pg in [1, 3, 4, 5, -1]:
		for i in range(20):
			...
			try:
				...
				line = u'%s\t%s\t%s\t%s\t%s\t%s\t%s' % (row_code, memo, phone_number, amount, unit_price, ip_addr, mod_date)
				print(line)
				f.write(line + '\n')
			except:
...
if __name__ == '__main__':
	with io.open('result.txt', 'w', encoding='utf8') as f:
		driver = webdriver.Chrome('C:\\forty\\chromedriver.exe')
		driver.implicitly_wait(3)
		# 블루홀 매매 페이지 오픈
		...
		print('--- sel ---')
		f.write(u'--- sel ---\n')
		crawl_frame(sel_frame_xpath, f)
		print('--- buy ---')
		f.write(u'--- buy ---\n')
		crawl_frame(buy_frame_xpath, f)
